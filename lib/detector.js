...
async function detectComponents(handsPath) {
  ...
  const claudePath = path.join(handsPath, '.claude');
...
/**
 * Detects skills: hands/.claude/skills/<name>/SKILL.md
 */
...
/**
 * Detects agents: hands/.claude/agents/<name>.md
 */
...
/**
 * Detects hooks: hands/.claude/hooks/<name>.json
 */
...
async function detectDependencyPool(handsPath) {
  ...
  const mcpPath = path.join(handsPath, '.claude', 'mcp-servers');
