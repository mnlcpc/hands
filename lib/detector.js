...
/**
 * Detects all user-facing components in the hands directory
 * @param {string} handsPath - Path to the hands directory
 * @returns {Promise<{skills: Object[], agents: Object[], hooks: Object[]}>}
 */
async function detectComponents(handsPath) {
  const components = {
    skills: [],
    agents: [],
    hooks: []
  ...
  if (!await fs.pathExists(handsPath)) {
    return components;
  }

  await Promise.all([
    detectSkills(handsPath, components),
    detectAgents(handsPath, components),
    detectHooks(handsPath, components)
  ]);
...
/**
 * Detects skills: hands/skills/<name>/SKILL.md
 */
async function detectSkills(handsPath, components) {
  const skillsPath = path.join(handsPath, 'skills');
...
/**
 * Detects agents: hands/agents/<name>.md
 */
async function detectAgents(handsPath, components) {
  const agentsPath = path.join(handsPath, 'agents');
...
/**
 * Detects hooks: hands/hooks/<name>.json
 */
async function detectHooks(handsPath, components) {
  const hooksPath = path.join(handsPath, 'hooks');
 ...
 */
async function detectDependencyPool(handsPath) {
  const pool = { mcpServers: [] };

  const mcpPath = path.join(handsPath, 'mcp');
  ...
  for (const entry of entries) {
    ...
    pool.mcpServers.push({
      ...
      category: 'mcpServers',
      sourcePath,
      targetFile: path.join('.claude', 'mcp.json'),
      config
    });
  }
