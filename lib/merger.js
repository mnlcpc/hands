...
async function mergeHook(targetFile, hookName, hookConfig) {
  ...
  for (const [eventType, handlers] of Object.entries(hookConfig)) {
    ...
    for (const handler of handlerArray) {
      // Add r4c marker to track which hooks came from rules4code
      const markedHandler = typeof handler === 'object'
        ? { ...handler, _r4c: hookName }
        : { command: handler, _r4c: hookName };

      // Check if this handler already exists (by _r4c marker)
...
/**
 * Removes hooks by name (removes all handlers with matching _r4c marker)
 * @param {string} targetFile - Path to settings.json
...
async function hasHook(targetFile, hookName) {
  ...
  for (const eventType of Object.keys(hooks)) {
    if (Array.isArray(hooks[eventType])) {
      const found = hooks[eventType].some(h => h._r4c === hookName);
...
/**
 * Syncs MCP servers: adds selected, removes unselected (that came from r4c)
 * @param {string} targetFile - Path to config.json
...
async function syncMcpServers(targetFile, selectedServers, allServers) {
  ...
  // Remove unselected servers that were installed by r4c
...
/**
 * Syncs hooks: adds selected, removes unselected (that came from r4c)
 * @param {string} targetFile - Path to settings.json
...
async function syncHooks(targetFile, selectedHooks, allHooks) {
  ...
  // Remove unselected hooks that were installed by r4c
